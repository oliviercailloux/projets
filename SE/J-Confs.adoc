...
JC:: Génération d’une demande de mission jeune chercheur (JC) (voir https://github.com/oliviercailloux/projets/blob/master/LAMSADE/demande_de_mission_jeune_chercheur.odt[canevas] JC). Le modèle est stocké dans le classpath et contient des champs d’utilisateur tels que Researcher_name ou Conference_name (dans LibreOffice : menu Insertion / Champ / Autres champs / Variables). Votre algorithme servira à mettre ces champs aux bonnes valeurs. (2)
