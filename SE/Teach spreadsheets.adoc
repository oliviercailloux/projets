...
Resources1:: Inclure dans votre class path un fichier de cours start-courses.csv. Créer une méthode main qui affiche le contenu du fichier.
Resources2:: Inclure dans votre class path un exemple de fichier de saisie des vœux. Créer un main qui ouvre le document à l’aide de Apache ODFToolkit (voir https://github.com/oliviercailloux/Test-ODFToolkit-ODS[Test ODFToolkit-ODS]) et le referme aussitôt, et affiche si le document a bien pu être ouvert.
